...
const todosAccess = new CompanyAccess()
const fileAccess = new FileAccess()

export async function getTodos(userId: string, sort: Sort = 'createdAt', next?: Next, limit?: number): Promise<{ items: CompanyItem[], next: Next }> {
...
export async function createTodo(userId: string, request: FromSchema<typeof CreateTodoRequest>): Promise<CompanyItem> {
  const todoId: string = uuid.v4()
  const todoItem: CompanyItem = {
    userId,
    todoId,
  ...
  await todosAccess.createCompany(todoItem)
